...
class Network:
    def __init__(self, dimensions): # пример измерений: 5, 4, 2
    ...
    def feed_forward(self, inputs):
        self.inputs = [i for i in inputs] # сохраняет входные данные для визуализации Hud
...
class RankableChromosome:
    ...
    def __lt__(self, other):
        """Позволяет сортировать хромосомы для выбора ранга по следующим правилам:
        - в верхней части списка отображается наивысшая контрольная точка
        - в случае одного и того же контрольно-пропускного пункта первым приезжает автомобиль, выдержавший большую дистанцию в списке"""
